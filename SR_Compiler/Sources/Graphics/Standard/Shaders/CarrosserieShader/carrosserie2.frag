...
void main()
{
        ...
        // ----------------------------------------------------------------------------------------------------------------------------------------------------
        vec4 diffuse = texture2D(uTexture,textureCoordinate);
        if (diffuse.b < 0.1)
                diffuse = uDiffuse;
        else
                diffuse = uDiffuseSecond;
        ...
        if (uReflectivity > 0.0){
                vec3 reflectedDirection = normalize(reflect(nNormal, nCamera));
                reflectedDirection.y = -reflectedDirection.y;
                vec4 reflectedColor = textureCube(uCubeTexture,-reflectedDirection.xyz);
                colorFinal =  colorFinal+reflectedColor*uReflectivity;
        }
